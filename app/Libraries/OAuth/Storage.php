...
class Storage implements AuthorizationCodeInterface, AccessTokenInterface, ClientCredentialsInterface, UserCredentialsInterface, RefreshTokenInterface, JwtBearerInterface, ScopeInterface, PublicKeyInterface, UserClaimsInterface
{
     ...
     * @return array|mixed
     */
    public function getClientDetails($client_id): mixed {
        $stmt = $this->db->prepare(sprintf('SELECT * from %s where client_id = :client_id',
            $this->config['client_table']));
        $stmt->execute(compact('client_id'));
     ...
     * @param array|null  $grant_types
     * @param string|null $scope
     * @param string|null $user_id
     * @return bool
     */
    public function setClientDetails(string $client_id, string $client_secret = null, string $redirect_uri = null, array $grant_types = null, string $scope = null, string $user_id = null): bool {
     ...
     * @return mixed
     */
    public function getAuthorizationCode($code): mixed {
        $stmt = $this->db->prepare(sprintf('SELECT * from %s where authorization_code = :code',
     ...
     * @return bool|mixed
     */
    public function setAuthorizationCode($code, $client_id, $user_id, $redirect_uri, $expires, $scope = null, $id_token = null): mixed {
        if(func_num_args() > 6) {
        ...
        if($this->getAuthorizationCode($code)) {
            $stmt = $this->db->prepare($sql = sprintf('UPDATE %s SET client_id=:client_id, user_id=:user_id, redirect_uri=:redirect_uri, expires=:expires, scope=:scope where authorization_code=:code',
                $this->config['code_table']));
        } else {
            $stmt = $this->db->prepare(sprintf('INSERT INTO %s (authorization_code, client_id, user_id, redirect_uri, expires, scope) VALUES (:code, :client_id, :user_id, :redirect_uri, :expires, :scope)',
                $this->config['code_table']));
        }
     ...
     * @return bool
     */
    private function setAuthorizationCodeWithIdToken($code, $client_id, $user_id, $redirect_uri, $expires, $scope = null, $id_token = null): bool {
        // convert expires to datestring
        $expires = date('Y-m-d H:i:s', $expires);

        // if it exists, update it.
        if($this->getAuthorizationCode($code)) {
            $stmt = $this->db->prepare($sql = sprintf('UPDATE %s SET client_id=:client_id, user_id=:user_id, redirect_uri=:redirect_uri, expires=:expires, scope=:scope, id_token =:id_token where authorization_code=:code',
                $this->config['code_table']));
        } else {
            $stmt = $this->db->prepare(sprintf('INSERT INTO %s (authorization_code, client_id, user_id, redirect_uri, expires, scope, id_token) VALUES (:code, :client_id, :user_id, :redirect_uri, :expires, :scope, :id_token)',
                $this->config['code_table']));
        }
    ...
    public function expireAuthorizationCode($code): bool {
        $stmt = $this->db->prepare(sprintf('DELETE FROM %s WHERE authorization_code = :code',
            $this->config['code_table']));
        ...
        foreach($validClaims as $validClaim) {
            if(in_array($validClaim, $claims)) {
                if($validClaim == 'address') {
                    ...
                    $userClaims['address'] = $this->getUserClaim($validClaim, $userDetails['address']
                        ? : $userDetails);
                } else {
     ...
     * @return bool|mixed
     */
    public function getRefreshToken($refresh_token): mixed {
        $stmt = $this->db->prepare(sprintf('SELECT * FROM %s WHERE refresh_token = :refresh_token',
    ...
    public function setRefreshToken($refresh_token, $client_id, $user_id, $expires, $scope = null): bool {
        ...
        $stmt = $this->db->prepare(sprintf('INSERT INTO %s (refresh_token, client_id, user_id, expires, scope) VALUES (:refresh_token, :client_id, :user_id, :expires, :scope)',
            $this->config['refresh_token_table']));
    ...
    public function unsetRefreshToken($refresh_token): bool {
        $stmt = $this->db->prepare(sprintf('DELETE FROM %s WHERE refresh_token = :refresh_token',
            $this->config['refresh_token_table']));
     ...
     * @return bool
     */
    protected function checkPassword(array $user, string $password): bool {
     ...
     * @return array|bool
     */
    public function getUser(string $username): bool|array {
        $apiUser = ApiUser::with('user')->whereUsername($username);

        return $apiUser->exists()
            ? array_merge([
                'user_id'  => $apiUser->first()->id,
                'password' => $apiUser->first()->user->password
            ], $apiUser->first()->toArray())
            : false;
    }
    ...
    public function setUser(string $username, string $password, string $firstName = null, string $lastName = null): bool {
        ...
        if($this->getUser($username)) {
            $stmt = $this->db->prepare($sql = sprintf('UPDATE %s SET password=:password, first_name=:firstName, last_name=:lastName where username=:username',
                $this->config['user_table']));
        } else {
            $stmt = $this->db->prepare(sprintf('INSERT INTO %s (username, password, first_name, last_name) VALUES (:username, :password, :firstName, :lastName)',
                $this->config['user_table']));
        }
    ...
    public function scopeExists($scope): bool {
        ...
        $stmt = $this->db->prepare(sprintf('SELECT count(scope) as count FROM %s WHERE scope IN (%s)',
            $this->config['scope_table'], $whereIn));
        $stmt->execute($scope);
    ...
    public function getDefaultScope($client_id = null): ?string {
        $stmt = $this->db->prepare(sprintf('SELECT scope FROM %s WHERE is_default=:is_default',
    ...
    public function getClientKey($client_id, $subject): string {
        $stmt = $this->db->prepare($sql = sprintf('SELECT public_key from %s where client_id=:client_id AND subject=:subject',
     ...
     * @return bool|null
     */
    public function getClientScope($client_id): ?bool {
    ...
    public function getJti($client_id, $subject, $audience, $expires, $jti) {
        $stmt = $this->db->prepare($sql = sprintf('SELECT * FROM %s WHERE issuer=:client_id AND subject=:subject AND audience=:audience AND expires=:expires AND jti=:jti',
     ...
     * @return bool
     */
    public function setJti($client_id, $subject, $audience, $expires, $jti): bool {
        $stmt = $this->db->prepare(sprintf('INSERT INTO %s (issuer, subject, audience, expires, jti) VALUES (:client_id, :subject, :audience, :expires, :jti)',
            $this->config['jti_table']));
    ...
    public function getPublicKey($client_id = null) {
        $stmt = $this->db->prepare($sql = sprintf('SELECT public_key FROM %s WHERE client_id=:client_id OR client_id IS NULL ORDER BY client_id IS NOT NULL DESC',
    ...
    public function getPrivateKey($client_id = null) {
        $stmt = $this->db->prepare($sql = sprintf('SELECT private_key FROM %s WHERE client_id=:client_id OR client_id IS NULL ORDER BY client_id IS NOT NULL DESC',
            $this->config['public_key_table']));
    ...
    public function getEncryptionAlgorithm($client_id = null): string {
        $stmt = $this->db->prepare($sql = sprintf('SELECT encryption_algorithm FROM %s WHERE client_id=:client_id OR client_id IS NULL ORDER BY client_id IS NOT NULL DESC',
            $this->config['public_key_table']));
     ...
     * @return string
     */
    public function getBuildSql(string $dbName = 'oauth2_server_php'): string {
        return "
